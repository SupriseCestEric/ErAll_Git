...
SpliceGraphs<-function(){
  ...
  for (item in args) local({
    ar=new("GeneModel", exonStart = data1[grep(toString(item), data1$transcript),4], exonEnd = data1[grep(toString(item), data1$transcript),5], dp = DisplayPars(color = "black"))
  ...
  maxcoords<-max(modelList[[1]]@exonEnd)+5000
  ...
  png(filename=toString(args[1]),width=1000,height=100*length(args))
